...
const EventCard = () => {
  ...
  const timeSlots = [
    { label: '09:30 AM - 12:30 PM', icon: <FaSun />, start: '09:30', end: '12:30' },
    { label: '01:00 PM - 04:00 PM', icon: <FaSun />, start: '13:00', end: '16:00' },
    { label: '04:30 PM - 06:00 PM', icon: <FaSun />, start: '16:30', end: '18:00' },
    { label: '06:30 PM - 09:30 PM', icon: <FaMoon />, start: '18:30', end: '21:30' },
    { label: '10:00 PM - 01:00 AM', icon: <FaMoon />, start: '22:00', end: '01:00' },
  ];
    ...
    <div className="flex flex-col p-4 space-y-8 lg:flex-row lg:justify-between lg:space-y-0 lg:space-x-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 lg:w-2/3">
        {events.map((event, index) => (
          ...
          >
            <img src={event.imageSrc} alt={event.title} className="w-40 h-40 object-fit" />
            <h2 className="mt-2 text-lg font-bold text-center">{event.title}</h2>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col items-center w-full lg:w-1/3">
        <div className="w-full p-6 bg-white border border-gray-300 rounded-lg">
          <h2 className="mb-4 text-xl font-bold text-center lg:text-left">Select Date & Time</h2>
          <input
            type="date"
          ...
          <div className="flex flex-col gap-2">
            {timeSlots.map((slot, index) => (
              <button
                ...
                } ${selectedTime === slot.label ? 'bg-primary text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
          ...
          <button
            className={`mt-6 w-full py-3 rounded-lg transition-colors duration-300 ${
              selectedTime ? 'bg-primary text-white hover:bg-primary-dark' : 'bg-gray-300 text-gray-700 cursor-not-allowed'
            }`}
